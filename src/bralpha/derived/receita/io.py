...
class ReceitaResearchInputMissingError(FileNotFoundError):
    pass
...
def gold_panel_root(paths: ResolvedPaths, panel: str) -> Path:
    root = paths.gold / "receita" / panel
    _assert_inside(root, paths.gold / "receita")
    return root
...
def read_parquet_root(
    ...
    *,
    ...
    if not files:
        if required:
            raise ReceitaResearchInputMissingError(f"Missing required parquet input: {root}")
        return None
