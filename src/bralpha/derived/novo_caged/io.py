...
class NovoCagedResearchInputMissingError(FileNotFoundError):
    pass
...
def gold_panel_root(paths: ResolvedPaths, panel: str) -> Path:
    root = paths.gold / "novo_caged" / panel
    _assert_inside(root, paths.gold / "novo_caged")
    return root
...
def read_parquet_root(
    ...
    *,
    ...
    if not files:
        if required:
            raise NovoCagedResearchInputMissingError(f"Missing required parquet input: {root}")
        return None
...
def _select_parquet_files(
    ...
    *,
    ...
    partition_dirs = _date_partition_dirs(root)
...
def _date_partition_dirs(root: Path) -> list[tuple[int, Path]]:
    partitions: list[tuple[int, Path]] = []
    for child in root.glob("**/*=*"):
        if not child.is_dir():
            continue
        key, _, raw_value = child.name.partition("=")
        if key not in {"year", "release_year"}:
            continue
        try:
            year = int(raw_value)
        except ValueError:
