...
class TesouroResearchInputMissingError(FileNotFoundError):
    pass
...
def gold_panel_root(paths: ResolvedPaths, panel: str) -> Path:
    root = paths.gold / "tesouro" / panel
    _assert_inside(root, paths.gold / "tesouro")
    return root
...
def read_parquet_root(
    ...
    *,
    ...
    if not files:
        if required:
            raise TesouroResearchInputMissingError(f"Missing required parquet input: {root}")
        return None
