...
class ANPResearchInputMissingError(FileNotFoundError):
    pass
...
def gold_panel_root(paths: ResolvedPaths, panel: str) -> Path:
    root = paths.gold / "anp" / panel
    _assert_inside(root, paths.gold / "anp")
    return root
...
def read_parquet_root(
    ...
    *,
    ...
    if not files:
        if required:
            raise ANPResearchInputMissingError(f"Missing required parquet input: {root}")
        return None
