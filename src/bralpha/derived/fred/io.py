...
class FredResearchInputMissingError(FileNotFoundError):
    pass
...
def gold_panel_root(paths: ResolvedPaths, panel: str) -> Path:
    root = paths.gold / "fred" / panel
    _assert_inside(root, paths.gold / "fred")
    return root
...
def read_parquet_root(
    ...
    *,
    ...
    if not files:
        if required:
            raise FredResearchInputMissingError(f"Missing required parquet input: {root}")
        return None
