...
class ONSResearchInputMissingError(FileNotFoundError):
    pass
...
def gold_panel_root(paths: ResolvedPaths, panel: str) -> Path:
    root = paths.gold / "ons" / panel
    _assert_inside(root, paths.gold / "ons")
    return root
...
def read_parquet_root(
    ...
    *,
    ...
    if not files:
        if required:
            raise ONSResearchInputMissingError(f"Missing required parquet input: {root}")
        return None
