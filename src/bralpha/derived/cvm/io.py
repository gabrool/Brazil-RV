...
class CVMResearchInputMissingError(FileNotFoundError):
    pass
...
def gold_panel_root(paths: ResolvedPaths, panel: str) -> Path:
    root = paths.gold / "cvm" / panel
    _assert_inside(root, paths.gold / "cvm")
    return root
...
def read_parquet_root(
    ...
    *,
    ...
    if not files:
        if required:
            raise CVMResearchInputMissingError(f"Missing required parquet input: {root}")
        return None
