...
def anp_paths(repo_root: Path) -> ResolvedPaths:
    return resolve_project_paths(repo_root, load_paths_config(repo_root))


def anp_dataset(repo_root: Path, dataset_id: str) -> DatasetConfig:
    return load_anp_dataset_registry(repo_root).get(dataset_id)


def anp_raw_store(paths: ResolvedPaths) -> RawStore:
    return RawStore(paths.raw, source="anp")


def anp_manifest_writer(paths: ResolvedPaths) -> ManifestWriter:
    return ManifestWriter(paths.manifests / "anp" / "downloads.jsonl")


def anp_bronze_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.bronze / "anp" / dataset_id


def anp_silver_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.silver / dataset_id


def download_anp_request(
    *,
    ...
    return ANPDownloadResult(record=record, raw_path=raw_path)
    ...
    *,
    ...
    for column in partition_cols:
        if column in work.columns:
            continue
        if column in {"year", "month", "snapshot_year"}:
