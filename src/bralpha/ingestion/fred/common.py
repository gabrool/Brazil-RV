...
def fred_paths(repo_root: Path) -> ResolvedPaths:
    return resolve_project_paths(repo_root, load_paths_config(repo_root))


def fred_dataset(repo_root: Path, dataset_id: str = "fred_series_observations") -> DatasetConfig:
    return load_fred_dataset_registry(repo_root).get(dataset_id)


def fred_raw_store(paths: ResolvedPaths) -> RawStore:
    return RawStore(paths.raw, source="fred")


def fred_manifest_writer(paths: ResolvedPaths) -> ManifestWriter:
    return ManifestWriter(paths.manifests / "fred" / "downloads.jsonl")


def fred_bronze_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.bronze / "fred" / dataset_id


def fred_silver_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.silver / dataset_id


def load_fred_series_config(repo_root: Path) -> list[FredSeriesConfig]:
    path = repo_root / "configs" / "series" / "fred.yaml"
    ...
    for item in series:
        ...
        if series_id in seen:
            duplicates.add(series_id)
        seen.add(series_id)
    if duplicates:
        raise ValueError(f"Duplicate FRED series_id(s): {sorted(duplicates)}")
    return series


def resolve_fred_api_key(api_key: str | None = None) -> str:
    key = (api_key if api_key is not None else os.environ.get(FRED_API_KEY_ENV, "")).strip()
    if not key:
        raise FredApiKeyMissingError(
            f"FRED live download requires {FRED_API_KEY_ENV} or an explicit api_key"
        )
    return key


def download_fred_request(
    *,
    ...
    try:
        response = client.get_bytes(url, params=params)
    ...
    return FredDownloadResult(record=record, raw_path=raw_path)
