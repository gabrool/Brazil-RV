...
def receita_paths(repo_root: Path) -> ResolvedPaths:
    return resolve_project_paths(repo_root, load_paths_config(repo_root))


def receita_dataset(repo_root: Path, dataset_id: str) -> DatasetConfig:
    return load_receita_dataset_registry(repo_root).get(dataset_id)


def receita_raw_store(paths: ResolvedPaths) -> RawStore:
    return RawStore(paths.raw, source="receita")


def receita_manifest_writer(paths: ResolvedPaths) -> ManifestWriter:
    return ManifestWriter(paths.manifests / "receita" / "downloads.jsonl")


def receita_bronze_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.bronze / "receita" / dataset_id


def receita_silver_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.silver / dataset_id


def download_receita_request(
    *,
    ...
    return ReceitaDownloadResult(record=record, raw_path=raw_path)


def failure_manifest_record(
    *,
    dataset: DatasetConfig,
    url: str,
    ...
    return ManifestRecord(
        ...
        source=dataset.source or "receita",
        source_url=url,
        request_params=request_params,
    ...
    for column in partition_cols:
        ...
        if column == "year":
            work = work.with_columns(
                pl.col(ref_date_col)
                .map_elements(_year_from_value, return_dtype=pl.Int64)
                .alias(column)
            )
