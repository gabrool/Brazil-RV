...
def ibge_paths(repo_root: Path) -> ResolvedPaths:
    return resolve_project_paths(repo_root, load_paths_config(repo_root))


def ibge_dataset(repo_root: Path, dataset_id: str) -> DatasetConfig:
    return load_ibge_dataset_registry(repo_root).get(dataset_id)


def ibge_raw_store(paths: ResolvedPaths) -> RawStore:
    return RawStore(paths.raw, source="ibge")


def ibge_manifest_writer(paths: ResolvedPaths) -> ManifestWriter:
    return ManifestWriter(paths.manifests / "ibge" / "downloads.jsonl")


def ibge_bronze_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.bronze / "ibge" / dataset_id


def ibge_silver_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.silver / dataset_id


def download_ibge_request(
    *,
...
def write_bronze_frame(
    ...
    *,
    ...
    if not partition_cols:
        return _write_bronze_part(frame, output_root, filename=filename, primary_keys=primary_keys)
