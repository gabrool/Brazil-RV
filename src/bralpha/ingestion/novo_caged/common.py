...
def novo_caged_paths(repo_root: Path) -> ResolvedPaths:
    return resolve_project_paths(repo_root, load_paths_config(repo_root))


def novo_caged_dataset(repo_root: Path, dataset_id: str) -> DatasetConfig:
    return load_novo_caged_dataset_registry(repo_root).get(dataset_id)


def novo_caged_raw_store(paths: ResolvedPaths) -> RawStore:
    return RawStore(paths.raw, source="novo_caged")


def novo_caged_manifest_writer(paths: ResolvedPaths) -> ManifestWriter:
    return ManifestWriter(paths.manifests / "novo_caged" / "downloads.jsonl")


def novo_caged_bronze_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.bronze / "novo_caged" / dataset_id


def novo_caged_silver_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.silver / dataset_id


def download_novo_caged_request(
    *,
    ...
    request_params = manifest_params or {}
    ...
    return NovoCagedDownloadResult(record=record, raw_path=raw_path)
    ...
    *,
    ...
    for column in partition_cols:
        if column in work.columns:
            continue
        if column in {"year", "month", "release_year"}:
            work = _with_derived_date_part(work, ref_date_col, column)
            derived_cols.add(column)
...
def _manifest_from_response(
    *,
    ...
    return ManifestRecord(
        ...
        source=dataset.source or "novo_caged",
