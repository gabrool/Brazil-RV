...
def tesouro_paths(repo_root: Path) -> ResolvedPaths:
    return resolve_project_paths(repo_root, load_paths_config(repo_root))


def tesouro_dataset(repo_root: Path, dataset_id: str) -> DatasetConfig:
    return load_tesouro_dataset_registry(repo_root).get(dataset_id)


def tesouro_raw_store(paths: ResolvedPaths) -> RawStore:
    return RawStore(paths.raw, source="tesouro")


def tesouro_manifest_writer(paths: ResolvedPaths) -> ManifestWriter:
    return ManifestWriter(paths.manifests / "tesouro" / "downloads.jsonl")


def tesouro_bronze_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.bronze / "tesouro" / dataset_id


def tesouro_silver_root(paths: ResolvedPaths, dataset_id: str) -> Path:
    return paths.silver / dataset_id


def download_tesouro_request(
    *,
    ...
    return TesouroDownloadResult(record=record, raw_path=raw_path)
...
def _manifest_from_response(
    *,
    ...
    return ManifestRecord(
        ...
        source=dataset.source or "tesouro",
