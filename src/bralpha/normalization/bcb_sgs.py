...
def normalize_sgs_to_silver(
    ...
    *,
    ...
    for row in bronze.to_dicts():
        ...
        available_date, model_usable = _availability(ref_date, config)
        rows.append(
            {
                "ref_date": ref_date,
...
def _availability(
    ...
    if config.availability_policy == "next_business_day":
        return next_business_day(ref_date), bool(config.model_usable)
    if config.availability_policy == "configured_lag_days":
        lag_days = config.availability_lag_days
        if lag_days is None:
            return None, False
        return ref_date + timedelta(days=lag_days), bool(config.model_usable)
    if config.availability_policy == "same_day":
        return ref_date, bool(config.model_usable)
    return None, False
