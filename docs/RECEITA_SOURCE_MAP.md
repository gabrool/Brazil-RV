...
- dados.gov.br metadata/API endpoint for Resultado da Arrecadacao: https://dados.gov.br/dados/api/publico/conjuntos-dados/resultado-da-arrecadacao

The visible dados.gov.br dataset page can render as a JavaScript-required landing page in non-browser clients. Live ingestion therefore discovers resources through the fixture-tested official metadata/API endpoint first. Static HTML link discovery is only a fallback when direct structured links are present in the returned page HTML; browser rendering and scraping are out of scope. PDF links are ignored and never parsed.
...
|---|---:|---|---|---|---|---|---|
| `receita_tax_collection_monthly` | P0 | live_download | Receita Arrecadacao category, dados.gov metadata/API endpoint, and `https://dados.gov.br/dados/conjuntos-dados/resultado-da-arrecadacao` | official structured tabular, CSV/TXT/ZIP preferred | monthly | `data/silver/receita_tax_collection_monthly/` | Resource discovery is fixture-tested against official metadata JSON first, with static HTML fallback only for direct structured links. The landing page may require JavaScript. PDFs are ignored. XLSX/ODS is unsupported unless a future official fixture requires `fastexcel`. |
