...
|---|---:|---|---|---|---|---|---|---|
| `bcb_sgs_series` | P0 | `live_download` | BCB SGS | `https://api.bcb.gov.br/dados/serie/bcdata.sgs.{codigo_serie}/dados` | JSON | mixed | `data/silver/bcb_sgs_series/` | Configured IDs cover rates, inflation, activity, credit, fiscal/debt, external/reserves, and monetary/liquidity metadata. Each row carries source reference URL, notes, availability basis, revision policy, and `model_usable`. Model-ready SGS currently includes Selic and daily international reserves; SGS IPCA is reference-only until matched to the official IBGE release calendar. |
...
- SGS configured series are limited to IDs verified through official BCB SGS API
  ...
  are documented. The exception promoted in issue #54 is daily international
  reserves `13982`, which uses conservative date-only next-business-day
  availability.
...
- Focus availability uses `Data` as the official weekly publication date and
  applies same-day EOD timing when the date is a B3 business day.
